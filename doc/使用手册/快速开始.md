...
## FPGA实现
可以使用`/fpga/`的示例工程，也可以自建工程。自建工程需要将`/rtl/`加入综合器的include目录。  
...
```
将`/rtl/config.v`中的`CPU_CLOCK_HZ`宏定义修改为处理器的时钟频率。如果使用自建的FPGA工程，需要手动将`PROG_FPGA_PATH`宏定义修改为`inst.txt`的文件路径。  
